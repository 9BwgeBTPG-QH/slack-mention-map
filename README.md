...
本ツールは分析対象チャンネルから以下のデータを Slack API 経由で取得します。

- メッセージの投稿者・@メンション先・スレッド参加者
- リアクションしたユーザー（絵文字の種類は記録しません）
- メッセージ先頭50文字（プレビュー用、ダッシュボードには表示されません）
- ユーザーの表示名・メールアドレス（ノード表示用）

**メッセージ本文の全文は収集・保存しません。**

### データの処理と保存

- すべての処理は**ローカルマシン上**で完了します。外部サーバーへのデータ送信は一切ありません
- 分析結果はメモリ上に保持され、アプリケーション終了時に破棄されます
- HTML エクスポートを利用した場合、集計済みのネットワークデータ（ユーザー名・メンション数・コミュニティ情報）がファイルに埋め込まれます

### ワークスペース管理者への推奨事項

- 本ツールの利用前に、分析対象チャンネルのメンバーに対してメンション関係の可視化を行うことを**事前に周知**してください
- エクスポートした HTML ファイルにはユーザー名とコミュニケーション構造が含まれるため、**共有範囲に注意**してください
- ワークスペースのデータポリシーや地域の個人情報保護法令（GDPR 等）に準拠した運用をお願いします

### Slack API の権限

本ツールが要求する権限は分析に必要な最小限のものです。詳細は[Slack App のセットアップ](#slack-app-のセットアップ)を参照してください。Bot はスラッシュコマンドの応答と DM 通知以外の書き込みは行いません。
